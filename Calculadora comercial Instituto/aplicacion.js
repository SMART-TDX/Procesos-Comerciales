(function (global) {
  ...
  async function downloadPreparedDocumentAsPdf(session, fileName) {
    ...
    try {
      ...
      var worker = global.html2pdf().set(pdfOptions);
      sheets.forEach(function (sheet, index) {
        // Cada hoja se rasteriza de forma independiente. Así html2pdf no puede
        // interpretar el espacio entre secciones como páginas A4 adicionales.
        worker = worker.from(sheet).toCanvas().toPdf();
