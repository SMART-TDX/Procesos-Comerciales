(function () {
  ...
  function confirmarBeneficios() {
    const cotizacion = construirCotizacion();
    if (estado.linea === "SMART_FLEX" && (!cotizacion || !cotizacion.beneficios.some(function (beneficio) { return beneficio.tipo === "EBOOK" && beneficio.esNinguno !== true; }))) {
      mostrarErrorPaso("Selecciona mínimo 1 E-Book para Smart Flex.");
      return;
    }
    avanzar();
  }
  ...
  function iniciarDesdeExperience(contexto, restaurado) {
    const linea = contexto && contexto.selectedProgram === "online" ? "SMART_ONLINE" : "SMART_FLEX";
    if (restaurado && typeof restaurado === "object" && restaurado.linea) {
      estado = Object.assign(estadoInicial(), restaurado);
    } else {
      estado = estadoInicial();
      estado.linea = linea;
      estado.paso = 1;
    }
    mostrarVista("asistente");
    renderizar();
    requestAnimationFrame(function () { $("#paso-actual").focus(); });
  }
  ...
  function manejarClick(evento) {
    ...
    if (accion.dataset.accion === "confirmar-beneficios") confirmarBeneficios();
  ...
  window.SmartSalesHubIntegration = Object.freeze({ iniciarDesdeExperience: iniciarDesdeExperience, obtenerEstado: obtenerEstadoIntegracion });
  iniciar();
  document.dispatchEvent(new CustomEvent("smart:hub-ready"));
})();
